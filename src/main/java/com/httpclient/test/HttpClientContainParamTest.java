...
public class HttpClientContainParamTest {

    /**
     * 使用HttpClient
     * 1、有参get请求
     * 2、比如参数中的密码存在特殊字符或者空格，浏览器会进行编码解码，但是HttClient不会
     * 如何解决？
     * 使用jdk的URLEncoder.encode做url encode,
     */
    @Test
    public void testContainParam() throws UnsupportedEncodingException {
        //1、可关闭的HttpClients客户端，相当于你打开一个浏览器
        CloseableHttpClient closeableHttpClient = HttpClients.createDefault();

        //get请求中带有参数
        String username = "123";
        String password = "123+456 qwe|789 ";
        //使用jdk的URLEncoder.encode做url encode,
        password = URLEncoder.encode(password, StandardCharsets.UTF_8.name());

        String url = "http://www.baidu.com/test?userName=" + username + "&password" + password;
